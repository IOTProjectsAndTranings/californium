...
public class SetupCalifornium {
    public enum DtlsCredentialsMode {
        PSK, RPK, X509
    }

    public enum DtlsSessionMode {
        NONE, CID, AUTO_RESUMPTION
    }
    ...
    static {
        ...
        CONFIG.setInt(Keys.DTLS_AUTO_RESUME_TIMEOUT, 30 * 1000); // 30s auto session resumption
    ...
    private String id;
    private DtlsCredentialsMode credentialsMode;
    private DtlsSessionMode sessionMode;

    private SslContextUtil.Credentials clientCredentials;
    private Certificate[] trustedCertificates;
    private boolean certificatesLoaded;

    public boolean setupEndpoints(InitMode force, String id, DtlsCredentialsMode credentialsMode, DtlsSessionMode sessionMode, SessionCache sessionCache) {
        ...
        Long autoResumptionTimeout = CONFIG.getOptLong(Keys.DTLS_AUTO_RESUME_TIMEOUT);
        Integer cidLength = CONFIG.getOptInteger(Keys.DTLS_CONNECTION_ID_LENGTH);

        if (force == InitMode.FORCE) {
            Log.i(LOG_TAG, "setup: forced! " + credentialsMode);
            ...
            Log.i(LOG_TAG, "setup: initialized = " + initialized + ", " + credentialsMode);
        }
        ...
        DtlsCredentialsMode effectiveCredentialsMode = credentialsMode;
        if (trustedCertificates == null && clientCredentials == null) {
            effectiveCredentialsMode = DtlsCredentialsMode.PSK;
        } else if (trustedCertificates == null && effectiveCredentialsMode == DtlsCredentialsMode.X509) {
            effectiveCredentialsMode = DtlsCredentialsMode.RPK;
        }

        DtlsSessionMode effectiveSessionMode = sessionMode;
        if (sessionMode == DtlsSessionMode.CID) {
            if (cidLength == null) {
                effectiveSessionMode = DtlsSessionMode.NONE;
            }
        } else if (sessionMode == DtlsSessionMode.AUTO_RESUMPTION) {
        ...
        boolean initDtls = !init || force == InitMode.FORCE_DTLS ||
                (effectiveSessionMode != this.sessionMode) ||
                (effectiveCredentialsMode != this.credentialsMode) ||
                (effectiveCredentialsMode == DtlsCredentialsMode.PSK && !id.equals(this.id));

        if (initDtls) {
            if (init) {
                if (effectiveSessionMode != this.sessionMode) {
                    Log.i(LOG_TAG, "setup: create DTLS endpoint with new session mode " + effectiveSessionMode);
                }
                if (effectiveCredentialsMode != this.credentialsMode) {
                    Log.i(LOG_TAG, "setup: create DTLS endpoint with new credentials mode " + effectiveCredentialsMode);
                }
                if (effectiveCredentialsMode == DtlsCredentialsMode.PSK && !id.equals(this.id)) {
                ...
                Log.i(LOG_TAG, "setup: create DTLS endpoint with credentialsMode " + effectiveCredentialsMode);
            }
            long time = System.nanoTime();
            DtlsConnectorConfig.Builder dtlsBuilder = new DtlsConnectorConfig.Builder();
            if (effectiveCredentialsMode == DtlsCredentialsMode.PSK) {
                dtlsBuilder.setPskStore(new PlugtestPskStore(id));
            } else if (effectiveCredentialsMode == DtlsCredentialsMode.RPK) {
            ...
            } else if (effectiveCredentialsMode == DtlsCredentialsMode.X509) {
            ...
            if (effectiveSessionMode == DtlsSessionMode.CID) {
                dtlsBuilder.setConnectionIdGenerator(new SingleNodeConnectionIdGenerator(cidLength));
            } else if (effectiveSessionMode == DtlsSessionMode.AUTO_RESUMPTION) {
                dtlsBuilder.setAutoResumptionTimeoutMillis(autoResumptionTimeout);
            }
            ...
            builder.setNetworkConfig(CONFIG);
            EndpointManager.getEndpointManager().setDefaultEndpoint(builder.build());
            this.sessionMode = effectiveSessionMode;
            this.credentialsMode = effectiveCredentialsMode;
            this.id = id;
            changed = true;
