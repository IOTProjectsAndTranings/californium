...
public class CoapContext {
    ...
    public void initCalifornium(SetupCalifornium.InitMode force) {
        try {
            ...
            SetupCalifornium.DtlsCredentialsMode dtlsCredentialsMode = SetupCalifornium.DtlsCredentialsMode.valueOf(dtlsCredentialsModeName);
            String dtlsSessionModeName = preferences.getString(PREF_DTLS_SESSION, SetupCalifornium.DtlsSessionMode.CID.name());
            SetupCalifornium.DtlsSessionMode dtlsSessionMode = SetupCalifornium.DtlsSessionMode.valueOf(dtlsSessionModeName);
            String uniqueID = preferences.getString(COAP_UNIQUE_ID, null);
            boolean changed = SetupCalifornium.getInstance().setupEndpoints(force, uniqueID, dtlsCredentialsMode, dtlsSessionMode, sessionCache);
            if (changed) {
